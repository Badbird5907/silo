...
const imageMimeTypes = Object.keys(image) as MimeType[];
...
export function isAllowedFileType(value: string): value is AllowedFileType {
  return (allowedFileTypes as readonly string[]).includes(value);
}
...
export function lookupMimeTypeFromFile(
  fileName: string,
  mimeType?: string | null,
): MimeType | undefined {
  const normalizedProvided = mimeType ? stripMimeParameters(mimeType) : "";

  if (normalizedProvided && isMimeType(normalizedProvided)) {
    return normalizedProvided;
  }

  const lookedUp = lookup(fileName);
