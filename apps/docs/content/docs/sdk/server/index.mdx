...
- `.input(schema)` attaches route-local input validation using Standard Schema v1
...
The builder is fully typed. The `context` type comes from `createSiloUpload<Request, UploadContext>()`, and per-route `input` types come from `.input(schema)`.

<Callout type="info">
`.input(schema)` is optional.

Routes that do not need app-defined input can omit it entirely.
</Callout>
...
When a route defines `.input(schema)`, middleware and route option resolvers receive the parsed schema output as `input`, including defaults and transforms from libraries like Zod.
...
f(["image"])
  ...
  .mimeTypes(async ({ context, input }) => {
    ...
    if (input?.kind === "pdf") return "application/pdf";
