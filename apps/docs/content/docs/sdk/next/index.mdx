...
If a route uses a function-based `.expects(...)` resolver, `extractRouterConfig(fileRouter)` cannot expose a static picker filter for that route. In that case, pass `accepts` to the React hook or component that opens the file picker.
