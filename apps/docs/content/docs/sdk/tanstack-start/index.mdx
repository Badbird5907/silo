...
If a route uses a function-based `.expects(...)` resolver, that extracted config cannot provide a stable picker filter for the client. Pass `accepts` to the React hook or component that opens the file picker when you need that filter.
