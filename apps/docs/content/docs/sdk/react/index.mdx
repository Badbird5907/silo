...
In that case, pass `accepts` to `useUpload(...)`, `useStagedUpload(...)`, `UploadButton`, or `UploadDropzone` if you want to control the file picker's accepted types.
</Callout>
...
That router config only contains statically known route expectations. If a route's `.expects(...)` value is a function, the React SDK cannot derive a picker `accepts` value from `extractRouterConfig(...)`, so you should provide `accepts` explicitly on the hook or component you use to open the picker.
