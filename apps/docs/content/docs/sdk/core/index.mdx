...
- file access and expiry updates
...
In production, `callbackUrl` must be an absolute public URL. You can pass it once when creating the core client, or override it per request.

The result shape depends on the mode:

- production returns prepared files plus registration info
- development can return an SSE stream for local workflows
...
- you need file listing or file update APIs
